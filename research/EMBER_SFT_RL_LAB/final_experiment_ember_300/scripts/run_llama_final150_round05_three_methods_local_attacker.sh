...
OUT_DIR="$EVAL_DIR/outputs/final150_round05_llama3_1_8b_base_prompt_agent_qwen3_5_9b_attacker_${STAMP}"
LOG_DIR="$ROOT/logs"
LOG_FILE="$LOG_DIR/final150_round05_llama3_1_8b_base_prompt_agent_qwen3_5_9b_attacker_${STAMP}.log"
PID_FILE="$LOG_DIR/final150_round05_llama3_1_8b.pid"
...
(
  env PYTHONUNBUFFERED=1 CUDA_VISIBLE_DEVICES=2,3 "$PYTHON" run_fiveway_exact_biasexpert_eval.py \
    --worker-mode \
    --worker-id 0 \
    "${COMMON_ARGS[@]}"
  env PYTHONUNBUFFERED=1 CUDA_VISIBLE_DEVICES=2,3 "$PYTHON" run_fiveway_exact_biasexpert_eval.py \
