...
RUN_NAME="${RUN_NAME:-vllm_first_eval150_round05_mistral}"
...
wait_vllm() {
  ...
  for _ in $(seq 1 300); do
    if curl -fsS "http://127.0.0.1:${port}/v1/models" >/dev/null 2>&1; then
      echo "[vllm-mistral] ${name} ready on port ${port}"
      return 0
    fi
    sleep 2
  done
  echo "[vllm-mistral] ${name} failed to become ready on port ${port}" >&2
  return 1
}
...
start_vllm_server() {
  ...
  shift 6
  echo "[vllm-mistral] starting ${served_name} gpu=${gpu} port=${port}"
  (
...
echo "[vllm-mistral] done: $RUN_ROOT"
