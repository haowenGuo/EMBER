...
RUN_NAME="${RUN_NAME:-vllm_first_eval150_round05_multiagent_${TARGET_LABEL}}"
...
wait_vllm() {
  ...
  for _ in $(seq 1 "$VLLM_READY_WAIT_STEPS"); do
    if curl -fsS "http://127.0.0.1:${port}/v1/models" >/dev/null 2>&1; then
      echo "[vllm-openmodel-multi] ${name} ready on port ${port}"
      return 0
    fi
    sleep 2
  done
  echo "[vllm-openmodel-multi] ${name} failed to become ready on port ${port}" >&2
  return 1
}
...
start_vllm_server() {
  ...
  shift 6
  echo "[vllm-openmodel-multi] starting ${served_name} gpu=${gpu} port=${port}"
  (
...
echo "[vllm-openmodel-multi] done: $RUN_ROOT"
