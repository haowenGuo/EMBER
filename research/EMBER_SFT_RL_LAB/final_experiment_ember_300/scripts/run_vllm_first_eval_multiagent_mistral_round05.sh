...
MISTRAL_TARGET="${MISTRAL_TARGET:-mistralai/Mistral-7B-Instruct-v0.3}"
...
ATTACKER_GPUS="${ATTACKER_GPUS:-1,3}"
...
RUN_NAME="${RUN_NAME:-vllm_first_eval150_round05_multiagent_mistral}"
...
wait_vllm() {
  ...
  for _ in $(seq 1 300); do
    if curl -fsS "http://127.0.0.1:${port}/v1/models" >/dev/null 2>&1; then
      echo "[vllm-mistral-multi] ${name} ready on port ${port}"
      return 0
    fi
    sleep 2
  done
  echo "[vllm-mistral-multi] ${name} failed to become ready on port ${port}" >&2
  return 1
}
...
start_vllm_server() {
  ...
  shift 6
  echo "[vllm-mistral-multi] starting ${served_name} gpu=${gpu} port=${port}"
  (
...
run_generation_for_mistral() {
  ...
  for offset in "${!target_gpu_list[@]}"; do
    ...
    start_vllm_server "$gpu" "$port" "$MISTRAL_TARGET" "target" "$LOG_DIR/mistral_target_gpu${gpu}.log" \
  ...
  "$PYTHON" "$SCRIPTS_DIR/vllm_generate_debates.py" \
    ...
    --output-dir "$out_dir" \
    > "$LOG_DIR/mistral_generation.log" 2>&1
...
run_bias_scoring() {
  ...
  for offset in "${!bias_gpu_list[@]}"; do
    ...
    start_vllm_server "$gpu" "$port" "$BIAS_EXPERT" "biasexpert" "$LOG_DIR/biasexpert_mistral_gpu${gpu}.log" \
  ...
  "$PYTHON" "$SCRIPTS_DIR/vllm_score_biasexpert.py" \
    ...
    > "$LOG_DIR/mistral_biasexpert_scoring.log" 2>&1
  stop_current_servers
}
...

{
  ...
  echo "mistral_target=$MISTRAL_TARGET"
...
start_attacker_servers
run_generation_for_mistral "$RUN_ROOT/mistral"
stop_current_servers

run_bias_scoring "$RUN_ROOT/mistral"

echo "[vllm-mistral-multi] done: $RUN_ROOT"
