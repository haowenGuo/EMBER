...
def resolve_repo_root(script_root):
    candidates = []
    env_root = os.environ.get("EMBER_REPO_ROOT")
    if env_root:
        candidates.append(Path(env_root))
    candidates.extend([script_root.parent, Path.cwd(), Path.cwd() / "EMBER_SFT_RL_LAB"])
    candidates.extend(script_root.parents)
    candidates.extend(
        [
            Path("/home/haowen/Lab/EMBER_SFT_RL_LAB"),
            Path("F:/lab/LLM_Eval/EMBER_SFT_RL_LAB"),
        ]
    )
    for candidate in candidates:
        if candidate and (candidate / "src" / "risk_gated_alignment").exists():
            return candidate
    return script_root.parent


REPO_ROOT = resolve_repo_root(SCRIPT_ROOT)
...
DEFAULT_MODEL_NAME = (
    "/data2/guohaowen_data/.cache/huggingface/hub/models--unsloth--Meta-Llama-3.1-8B-Instruct/"
    "snapshots/a2856192dd7c25b842431f39c179a6c2c2f627d1"
)
DEFAULT_SFT_CHECKPOINT = "/home/haowen/Lab/EMBER_SFT_RL_LAB/runs/llama3_1_8b_instruct_qwen_protocol/sft/best"
DEFAULT_RL_CHECKPOINT = "/home/haowen/Lab/EMBER_SFT_RL_LAB/runs/llama3_1_8b_instruct_qwen_protocol/rl/final"
DEFAULT_CMV_PATH = REPO_ROOT / "source_data" / "changemyview_persuasion_kto.jsonl"
DEFAULT_OUTPUT_DIR = SCRIPT_ROOT / "outputs" / "llama_qwen_protocol_exact_biasexpert_fiveway_cmv101_200_fullcontext"
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="EMBER-AGENT rewrites only when verifier total score is at least this value.",
    )
    parser.add_argument(
        "--agent-max-refinement-rounds",
        type=int,
        default=2,
        help="Maximum external-verifier rewrite attempts per target response.",
    )
    parser.add_argument(
        "--agent-min-response-chars",
        type=int,
        default=AGENT_MIN_RESPONSE_CHARS,
        help="Reject agent rewrite candidates shorter than this many characters.",
    )
    parser.add_argument(
        "--max-input-length",
        type=int,
