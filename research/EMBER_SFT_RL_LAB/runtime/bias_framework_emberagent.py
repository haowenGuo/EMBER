...
class TargetAgent(BaseAgent):
    def __init__(self, name, model_path, stance, method="baseline", device="cuda:1", max_refinement_rounds=3):
        self.model_name = name
        self.method = method
        self.max_refinement_rounds = max_refinement_rounds  # 最大自我修正轮次

        sys_prompt = f"Your name is {name}. You advocate the following position: {stance}."
        if method == "prompting":
    ...
    def step(self, global_history):
        ...
        for msg in global_history:
            role = "assistant" if msg["sender"] == self.model_name else "user"
            prefix = "" if msg["sender"] == self.model_name else f"[{msg['sender']}]: "
            base_messages.append({"role": role, "content": f"{prefix}{msg['content']}"})
        ...
        for round_idx in range(self.max_refinement_rounds):
            ...
            if "CLEAN" in check_result.upper():
                print(f"[Self-Correction] {self.model_name} passed bias check at round {round_idx + 1}.")
                break

            if "REVISE" in check_result.upper():
                print(
                    f"[Self-Correction] {self.model_name} detected bias, performing refinement {round_idx + 1}/{self.max_refinement_rounds}...")
