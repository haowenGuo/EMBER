...
def main():
    ...
    results_file = "bias_experiment_results_GPT_multiagent_mitigation.jsonl"
    max_rounds=10
    i=0
    device="cuda:0"
    ...
    for model_path in config.MODELS_TO_TEST:
        for method in ["prompting"]:  # 暂不包含训练方法，逻辑一致
            ...
            for data in dataset:
                ...
                print("------------------------------")
                target_agent = TargetAgent(name="GPT",model_path=model_path, stance=data["initial_context"], method=method,device=device)

                # 3. 设置对手 (针对多智能体场景，可放多个 ProvocateurAgent)
                # 二智能体辩论：一个对手；多智能体讨论：多个对手
                provocateurs = [ProvocateurAgent(name="d0",helpermodel_name1="d1",helpermodel_name2="d2",targetmodel_name=target_agent.model_name,model_path=config.MODELS[0], stance=data["initial_context"], persona_type="aggressive",device=device),ProvocateurAgent(name="d1",helpermodel_name1="d0",helpermodel_name2="d2",targetmodel_name=target_agent.model_name,model_path=config.MODELS[0], stance=data["initial_context"], persona_type="aggressive",device=device),ProvocateurAgent(name="d2",helpermodel_name1="d0",helpermodel_name2="d1",targetmodel_name=target_agent.model_name,model_path=config.MODELS[0], stance=data["initial_context"], persona_type="aggressive",device=device),]
